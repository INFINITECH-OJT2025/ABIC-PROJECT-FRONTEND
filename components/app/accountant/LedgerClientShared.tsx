...
function ClientLedgerPage({ role }: { role: "superadmin" | "accountant" }) {
    ...
    const exportMainLedger = () => {
        if (!entries || entries.length === 0) {
            showToast("Info", "No data to export for Main Ledger.", "info");
            return;
        }

        import("xlsx-js-style").then((m) => {
            const XLSX = m.default || m;
            const currentOwnerName = owners.find(o => String(o.id) === String(selectedOwnerId))?.name ?? "Client";
            ...
            ws['!cols'] = [
                { wch: 15 }, { wch: 20 }, { wch: 25 }, { wch: 35 }, { wch: 80 }, { wch: 18 }, { wch: 18 }, { wch: 20 }, { wch: 30 }, { wch: 30 }
            ];
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Main Ledger");
            XLSX.writeFile(wb, `${sanitizedName}_main_ledger.xlsx`);
        });
    };
    ...
    const exportUnitBudgets = async () => {
        ...
        try {
            ...
            const unitPromises = activeBudget.units.map(async (unit) => {
                const res = await fetch(`/api/accountant/ledger/clients?owner_id=${selectedOwnerId}&unit_id=${unit.id}&sort=oldest`);
                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-gray-100 gap-4">
                        <div className="flex flex-col space-y-4">
                            <div>
                                <h2 className="text-xl font-bold text-[#5f0c18]">Client Financials</h2>
                            ...
                            <div>
                                ...
                                <div className="text-sm font-bold text-gray-500 mt-1 uppercase flex items-center gap-1.5 flex-wrap">
                                    {ownersLoading ? "Loading..." : (
                                        <>
                                            <span>{owners.find(o => String(o.id) === String(selectedOwnerId))?.name ?? "Select Client"}</span>
